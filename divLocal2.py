...
for x in range(0, num_paginas):
    ...
    for minTest in range(np.max(hist_ver_filtrado[x]) -1, 0, -1):
        ...
        if len(ini_filas) == (filas_txt[x] + margen):
            minimo[x] = minTest
...
match = np.zeros((margen, num_paginas))
for x in range(0, num_paginas):
    for y in range(0, filas_txt[x]):

        for z in range(0, margen):

            match[z][x] += abs(txt[x][y][1] - num_palabras[x][y + z])
...
for x in range(0, num_paginas):
    for y in range(0, num_filas[x]):
        ...
        cv2.line(original, (filas[x][2], filas[x][0][y]), (filas[x][3], filas[x][0][y]), (255, 0, 0), 1)
        cv2.line(original, (filas[x][2], filas[x][1][y]), (filas[x][3], filas[x][1][y]), (255, 0, 0), 1)
...
filestring = '../../Osborne/div2_%s_CC.png' % nombre
