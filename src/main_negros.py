...
for y in range(0, num_filas):
    ...
    for region in measure.regionprops(label_image):
        ...
        rect = mpatches.Rectangle((minc, minr + ini_filas[y]), maxc - minc, maxr - minr, fill=False, edgecolor='red',
                                  linewidth=1)
        ax.add_patch(rect)
        ...
        texto = str(z)
        cv2.putText(original, texto, (minc, maxr + ini_filas[y] + 25), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 1,
                    cv2.LINE_AA)
...
ratio_negro = np.zeros(tam_groundtruth)

aciertos = 0

'''
# Para caja del groundtruth
for x in range(0, tam_groundtruth):

    print("Comparando alabra %d de %d" % (x, tam_groundtruth))

    # La comparamos con cada caja del programa
    for z in range(0, len(res)):
        ...
        # Si las cajas interseccionan
        if SI > 0:
            SA = (XA2 - XA1) * (YA2 - YA1)
            SB = (XG2 - XG1) * (YG2 - YG1)
            negroA = np.sum(img[YA1:YA2, XA1:XA2])
            negroG = np.sum(img[YG1:YG2, XG1:XG2])

            # Calcular el ratio de intersección
            ratio[x] = np.max([ratio[x], SI / (SA + SB - SI)])
            ratio_negro[x] = np.max([ratio_negro[x], np.min([negroA, negroG])/np.max([negroA, negroG])])


    if ratio[x] > 0.3:

        if ratio_negro[x] > 0.9:

            aciertos = aciertos + 1


print("149: %d aciertos de %d \n%f por ciento de efectividad" % (aciertos, tam_groundtruth, aciertos / tam_groundtruth * 100))
'''

for x in range(0, tam_groundtruth):

    z = int(groundtruth[x][2])
    ...
    SI[x] = np.max([0, (np.min([XA2, XG2]) - np.max([XA1, XG1]))]) * np.max([0, (np.min([YA2, YG2]) - np.max([YA1, YG1]))])
    SA[x] = (XA2 - XA1) * (YA2 - YA1)
    SB[x] = (XG2 - XG1) * (YG2 - YG1)

    ratio[x] = SI[x]/(SA[x]+SB[x]-SI[x])

    negroA = np.sum(img[YA1:YA2, XA1:XA2])
    negroG = np.sum(img[YG1:YG2, XG1:XG2])
    ratio_negro[x] = np.max([ratio_negro[x], np.min([negroA, negroG]) / np.max([negroA, negroG])])

    #print("Ratio: %f, SA: %d, SB: %d, SI: %d" % (ratio[x], SA[x], SB[x], SI[x]))

    if ratio_negro[x] > 0.8:

        aciertos = aciertos + 1


print("%d aciertos de %d \n%f por ciento de efectividad" % (aciertos, tam_groundtruth, aciertos/tam_groundtruth*100))
